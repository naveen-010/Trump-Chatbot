...
def stt():
    while True:
        ...
        file_path = f"Temporary_files/User/audio {n}.wav"  
...
def playtts(text):
    ...
    if response.status_code == 200:
        ...
        n = len(os.listdir('Temporary_files')) + 1
        with open(f"Temporary_files/Trump/audio {n}.wav", "wb") as f:
            f.write(response.content)
        audio = AudioSegment.from_file(f"Temporary_files/Trump/audio {n}.wav")
        audio_duration = len(audio)/1000
        play_audio(f"Temporary_files/Trump/audio {n}.wav")
