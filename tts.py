...
command = "jp2a /home/zoe/Downloads/trump.jpg --colors --chars='01'"
...
def stt():
    while True:
        n = len(os.listdir('Temporary files')) + 1
        file_path = f"/home/zoe/Jupyter/Python/AI Chatbot/Temporary files/Naveen {n}.wav"  
        record_audio(file_path)

        text = transcribe_audio(file_path)
        print(text)
        continue_recording = input("Do you want to record again? (y/n): ")
        print('\033[F','\r', ' '*37, '\r',  flush = True,sep = '', end = '') # to clear input line

        if continue_recording.lower() == 'n':
            return text
        else:
...
def playtts(text):
    ...
    refaudio = open('/home/zoe/Videos/obs/trump_speech.mkv' , 'rb') #trump
    ...
    if response.status_code == 200:
        ...
        n = len(os.listdir('Temporary files')) + 1
        with open(f"/home/zoe/Jupyter/Python/AI Chatbot/Temporary files/Trump {n}.wav", "wb") as f:
            f.write(response.content)
        audio = AudioSegment.from_file(f"/home/zoe/Jupyter/Python/AI Chatbot/Temporary files/Trump {n}.wav")
        audio_duration = len(audio)/1000
        play_audio(f"/home/zoe/Jupyter/Python/AI Chatbot/Temporary files/Trump {n}.wav")
...
conversation_history = [
    {"role": "system", "content": "You are Trump.You remember the user's previous questions. Roast the user brutally, NO MERCY , in under 40-50 words."}
]
